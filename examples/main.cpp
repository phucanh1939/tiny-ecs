...
Target findClosestEnemy(tinyecs::World& world, const Position& position, const Team& team, float range) {
    Target result{.entity = {}, .distanceSquared = std::numeric_limits<float>::max()};
    const float rangeSquared = range * range;

    for (auto [entity, otherPosition, otherTeam, health] : world.query<Position, Team, Health>()) {
        ...
        const float distance = distanceSquaredBetween(position, otherPosition);
...
class IdleSystem {
public:
    void update(tinyecs::World& world, tinyecs::CommandBuffer& commands, float deltaTime) {
        for (auto [entity, idle, position, team, sensor] : world.query<Idle, Position, Team, Sensor>()) {
            idle.remainingTime -= deltaTime;

            const Target target = findClosestEnemy(world, position, team, sensor.range);
...
public:
    void update(tinyecs::World& world, tinyecs::CommandBuffer& commands, float deltaTime) {
        for (auto [entity, wander, position, velocity, team, sensor, spawn] : world.query<Wander, Position, Velocity, Team, Sensor, SpawnPosition>()) {
            ...
            if (wander.remainingTime <= 0.0f) {
                wander.targetPosition = randomPositionAroundSpawn(spawn);
                wander.remainingTime = 3.0f;
            }

            moveTowards(position, velocity, wander.targetPosition);
        }
    }
};
...
public:
    void update(tinyecs::World& world, tinyecs::CommandBuffer& commands, float deltaTime) {
        for (auto [entity, chasing, position, velocity, sensor, attack] : world.query<Chasing, Position, Velocity, Sensor, Attack>()) {
            if (!world.isValid(chasing.target)) {
                startReturning(entity, velocity, commands);
                continue;
            }

            const Position& targetPosition = world.getComponent<Position>(chasing.target);
            const float distance = distanceSquaredBetween(position, targetPosition);
            const float attackRangeSquared = attack.range * attack.range;

            if (distance <= attackRangeSquared) {
                velocity = {};
                commands.removeComponent<Chasing>(entity);
                commands.addComponent(entity, Attacking{chasing.target, 0.0f});
                continue;
            }

            const float chaseRangeSquared = sensor.range * sensor.range * 4.0f;
...
private:
    void startReturning(Entity entity, Velocity& velocity, tinyecs::CommandBuffer& commands) {
        velocity = {};
        commands.removeComponent<Chasing>(entity);
...
public:
    void update(tinyecs::World& world, tinyecs::CommandBuffer& commands, float deltaTime) {
        for (auto [entity, returning, position, velocity, team, sensor, spawn] : world.query<Returning, Position, Velocity, Team, Sensor, SpawnPosition>()) {
            const Target target = findClosestEnemy(world, position, team, sensor.range);
            ...
            const Position spawnPosition{spawn.x, spawn.y};
            const float distance = distanceSquaredBetween(position, spawnPosition);
            ...
            moveTowards(position, velocity, spawnPosition);
        }
    }
};
...
public:
    void update(tinyecs::World& world, tinyecs::CommandBuffer& commands, float deltaTime) {
        for (auto [entity, attacking, attack, position] : world.query<Attacking, Attack, Position>()) {
            ...
            const Position& targetPosition = world.getComponent<Position>(attacking.target);
            const float distance = distanceSquaredBetween(position, targetPosition);

            if (distance > attack.range * attack.range) {
                commands.removeComponent<Attacking>(entity);
                commands.addComponent(entity, Chasing{attacking.target});
                continue;
            }
            ...
            if (attacking.cooldown <= 0.0f) {
                ...
                Health& targetHealth = world.getComponent<Health>(attacking.target);
...
class DeathSystem {
public:
    void update(tinyecs::World& world, tinyecs::CommandBuffer& commands) {
...
int main() {
    ...
    while (true) {
        ...
        idleSystem.update(world, idleCommands, deltaTime);
        wanderSystem.update(world, wanderCommands, deltaTime);
        chasingSystem.update(world, chasingCommands, deltaTime);
        returningSystem.update(world, returningCommands, deltaTime);
        attackSystem.update(world, attackCommands, deltaTime);
        deathSystem.update(world, deathCommands);
